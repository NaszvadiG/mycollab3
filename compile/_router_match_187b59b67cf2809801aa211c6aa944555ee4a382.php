<?php
  ...
  return function ($path, $query_string) {
    $matches = null;
    if(preg_match('/^projects\\/([a-z0-9\\-\\._]+)\\/milestones\\/(\\d+)\\/notebooks$/', $path, $matches)) {
  ...
  0 => 'project_slug',
    ...
    } elseif(preg_match('/^admin\\/indices\\/object-contexts\\/rebuild\\/discussions$/', $path, $matches)) {
  ...
  0 => 'role_id',
    ...
    } elseif(preg_match('/^data-sources\\/popup$/', $path, $matches)) {
      return Router::doMatch($path, 'data_source_popup', 'data-sources/popup', array ( 'controller' => 'data_sources', 'action' => 'popup', 'module' => 'data_sources', ), array(), $matches, $query_string);
  ...
  0 => 'label_id',
    ...
    } elseif(preg_match('/^public\\/subscriptions\\/unsubscribe$/', $path, $matches)) {
      return Router::doMatch($path, 'public_subscriptions_unsubscribe', 'public/subscriptions/unsubscribe', array ( 'controller' => 'public_subscriptions', 'action' => 'unsubscribe', 'module' => 'system', ), array(), $matches, $query_string);
