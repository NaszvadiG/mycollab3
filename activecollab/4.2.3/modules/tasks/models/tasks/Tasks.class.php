<?php
  ...
  class Tasks extends ProjectObjects {
    ...
    static function findForOutline(Project $project, $milestone_id, User $user, $state = STATE_VISIBLE) {
      ...
      foreach ($tasks as $subobject) {
        ...
        $results[] = array(
          ...
          'category_id'					=> array_var($subobject, 'category_id', null),
          'class'               => 'Task',
    ...
    static function findForPrint(Project $project, $min_state = STATE_VISIBLE, $min_visibility = VISIBILITY_NORMAL, $group_by = null, $filter_by = null) { 	
	  ...
	  static function findForCalendarByUser(User $user, $from = null, $to = null, $assigned = false, $all_for_admins_and_pms = false, $include_completed_and_archived = false) {
		  ...
		  if ($include_completed_and_archived) {
			  $conditions[] = DB::prepare('(state = ? OR state = ?)', STATE_ARCHIVED, STATE_VISIBLE);
		  } else {
		  ...
		  $conditions = implode(" AND ", $conditions);
 		  $tasks = DB::execute("SELECT id, name, project_id, due_on, integer_field_1 as task_id, completed_on, state FROM $project_objects_table WHERE $conditions");

		  // return false if there is no tasks found
		  if (!$tasks) {
			  return false;
		  } // if
		  ...
		  if (is_foreachable($tasks)) {
			  ...
			  foreach ($tasks as $subobject) {
				  ...
				  $task_id = array_var($subobject, 'task_id');
				  $project_id = array_var($subobject, 'project_id');
				  $state = array_var($subobject, 'state');
				  $completed_on = array_var($subobject, 'completed_on');

				  if ($state != STATE_VISIBLE && $state != STATE_ARCHIVED) {
					  continue;
				  } // if
    ...
    static function getMyLateTasksFilter(User $user) {
      ...
      $filter->setDueOnFilter(DataFilter::DATE_FILTER_LATE);
