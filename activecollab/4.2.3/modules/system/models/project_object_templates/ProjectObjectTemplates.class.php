		  ...
		  $objects = DB::execute('SELECT id, type, template_id, parent_id, value, position FROM ' . TABLE_PREFIX . 'project_object_templates WHERE id IN (?) ORDER BY position', $object_ids);
			...
			if (is_foreachable($objects)) {
				...
				foreach($objects as $subobject) {
					$subobject_id = array_var($subobject, 'id');
					$subobject_type = array_var($subobject, 'type');

					$subobject = array_merge($subobject, unserialize(array_var($subobject, "value")));
					...
					$data = array(
						...
						'estimate'            => null,
						'urls'                => array(
							'view'                => str_replace('--OBJECT-ID--', $subobject_id, str_replace('--OBJECT-TYPE--', strtolower($subobject_type), $view_object_url_pattern)),
							'edit'                => str_replace('--OBJECT-ID--', $subobject_id, str_replace('--OBJECT-TYPE--', strtolower($subobject_type), $edit_object_url_pattern)),
							'delete'              => str_replace('--OBJECT-ID--', $subobject_id, str_replace('--OBJECT-TYPE--', strtolower($subobject_type), $delete_object_url_pattern))
						),
						'permissions'         => array(
							'can_edit'            => true,
							'can_delete'          => true
					...
					switch ($subobject_type) {
						...
						case "Task":
							$data = array_merge($data, array(
								'category_id'     => array_var($subobject, 'category_id'),
								'visibility'      => array_var($subobject, 'visibility'),
								'milestone_id'    => array_var($subobject, 'parent_id'),
								'event_names'     => array(
									'updated'         => 'task_template_updated'
								),
								'estimate'        => array(
									'value'           => (float) array_var($subobject, 'estimate_value'),
									'job_type_id'     => array_var($subobject, 'estimate_job_type_id'),
								),
							));
						...
						case 'Subtask':
							...
							$subtask_parent_id = array_var($subobject, 'parent_id');
	  ...
	  static function findPositionsForList(ProjectTemplate $template) {
		  ...
		  if (is_foreachable($positions)) {
			  ...
			  foreach ($positions as $subobject) {
				  $position_id = array_var($subobject, 'id');
					$podition_data = unserialize(array_var($subobject, 'value'));
				  $assigned = Users::findById(array_var($podition_data, 'user_id'));
				  ...
				  $results[] = array_merge($data, $podition_data);
			  } // foreach
		  } // if
	  ...
	  static function findCategoriesForList(ProjectTemplate $template, $category_type = null) {
		  ...
		  if (is_foreachable($categories)) {
			  ...
			  foreach ($categories as $subobject) {
				  $category_id = array_var($subobject, 'id');
				  $category_data = unserialize(array_var($subobject, 'value'));

				  $data = array(
					  'id'          => $category_id,
					  'type'        => "Category",
					  'subtype'     => array_var($subobject, 'subtype'),
					  'urls'        => array(
	  ...
	  static function findFilesForList(ProjectTemplate $template) {
		  ...
		  if (is_foreachable($files)) {
			  ...
			  foreach ($files as $subobject) {
				  $file_id = array_var($subobject, 'id');
				  $file_data = unserialize(array_var($subobject, 'value'));

				  $data = array(
					  'id'          => $file_id,
					  'type'        => 'File',
					  'file_size'   => format_file_size(array_var($subobject, 'file_size')),
