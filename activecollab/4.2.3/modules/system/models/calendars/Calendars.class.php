		  ...
		  if ($calendars) {
			  if (is_foreachable($calendars)) {
				  ...
				  foreach ($calendars as $subobject) {
					  ...
					  $created_by_id = (integer) array_var($subobject, 'created_by_id');
					  $is_creator = $created_by_id == $user->getId() ? true : false;

					  // if you are not creator you can only change color of calendar
					  if (!$is_creator) {
					  ...
					  $result[] = array(
						  ...
						  'created_by_name' => array_var($subobject, 'created_by_name'),
						  'permissions' => array(
							  'can_edit'    => true,
							  'can_trash'   => $is_creator
						  ),
						  'urls'        => array(
