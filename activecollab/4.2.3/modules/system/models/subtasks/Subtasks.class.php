    ...
    static function findTypeIdMapOfPotentialParents(Project $project, $min_state = STATE_ARCHIVED) {
	  ...
	  static function findForCalendarByUser(User $user, $from = null, $to = null, $assigned = false, $all_for_admins_and_pms = false, $include_completed_and_archived = false) {
		  ...
		  if ($include_completed_and_archived) {
			  $task_conditions[] = DB::prepare('(state = ? OR state = ?)', STATE_ARCHIVED, STATE_VISIBLE);
		  } else {
		  ...
		  if (is_foreachable($tasks)) {
			  foreach ($tasks as $subobject) {
				  ...
				  $task_ids[] = $task_id;
				  $task_id_map[$task_id] = array_var($subobject, 'task_id');
				  $task_id_project_id_map[$task_id] = array_var($subobject, 'project_id');
			  } // foreach
		  } // if
		  ...
		  // add completed and archived condition
		  if ($include_completed_and_archived) {
			  $subtask_conditions[] = DB::prepare('(state = ? OR state >= ?)', STATE_ARCHIVED, STATE_VISIBLE);
		  } else {
		  ...
		  $subtasks = DB::execute("SELECT id, body as name, parent_id, due_on, state, completed_on FROM $subtasks_table WHERE $subtask_conditions");
		  ...
		  if (is_foreachable($subtasks)) {
			  ...
			  foreach ($subtasks as $subobject) {
				  ...
				  $task_id = array_var($subobject, 'parent_id');
