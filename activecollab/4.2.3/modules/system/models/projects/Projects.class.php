<?php
  ...
  class Projects extends BaseProjects {
    ...
    static function canAdd(User $user) {
      return $user->isAdministrator() || $user->isProjectManager() || $user->getSystemPermission('can_add_projects');
    } // canAdd
    ...
    static function findForTimeline(User $user) {
		  ...
		  if (is_foreachable($projects)) {
			  foreach ($projects as $subobject) {
				  $project_id = array_var($subobject, 'id');
				  $project_slug = array_var($subobject, 'slug');
				  $start_on = array_var($subobject, 'start_on');
				  $due_on = array_var($subobject, 'due_on');
				  $completed_on = array_var($subobject, 'completed_on');

				  list($total, $open) = ProjectProgress::getProjectProgress($project_id);
				  $completed = $total - $open;
				  if($total && $completed) {
					  $percents_done = floor($completed / $total * 100);
				  } else {
					  $completed_on = array_var($subobject, 'completed_on');
					  $percents_done = $completed_on instanceof DateValue ? 100 : 0;
				  } // if

				  $leader = Users::findById(array_var($subobject, 'leader_id'));
        ...
        if(is_foreachable($available_project_ids)) {
          ...
          if(is_foreachable($parents_map)) {
            ...
            if(is_foreachable($changed_parent_ids)) {
              $updated_parent_project_ids = DB::executeFirstColumn("SELECT DISTINCT $projects_table.id FROM $projects_table, $project_objects_table WHERE $projects_table.id = $project_objects_table.project_id AND $project_objects_table.id IN (?)", implode(', ', array_unique($changed_parent_ids)));
            } // if
          } // if
