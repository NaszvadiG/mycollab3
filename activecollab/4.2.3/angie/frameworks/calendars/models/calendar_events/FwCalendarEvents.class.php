<?php
  ...
  abstract class FwCalendarEvents extends BaseCalendarEvents {
	  ...
	  static function canAdd(User $user, Calendar $calendar) {
		  return $user->getId() == $calendar->getCreatedById() || ($calendar->canView($user) && $calendar->getShareCanAddEvents());
	  } // canAdd
	  ...
	  static function findTrashed(User $user, &$map) {
		  ...
		  if (is_foreachable($skip_calendar_ids)) {
			  $trashed_calendar_events = DB::execute('SELECT id, name FROM ' . TABLE_PREFIX . 'calendar_events WHERE state = ? AND parent_id NOT IN (?) ORDER BY created_on DESC', STATE_TRASHED, $skip_calendar_ids);
		  } else {
			  $trashed_calendar_events = DB::execute('SELECT id, name FROM ' . TABLE_PREFIX . 'calendar_events WHERE state = ? ORDER BY created_on DESC', STATE_TRASHED);
		  } // if
	  ...
	  static function findForList(User $user, DateValue $from, DateValue $to=null) {
		  ...
		  $calendar_ids = Calendars::getCalendarIdsByUser($user, STATE_VISIBLE);
		  ...
		  if (is_foreachable($events)) {
			  ...
			  foreach($events as $subobject) {
				  $id = array_var($subobject, 'id');
				  $parent_id = array_var($subobject, 'parent_id');

				  // is calendar event creator
				  $calendar_event_created_by_id = array_var($subobject, 'created_by_id', 0);
				  $is_calendar_event_creator = $calendar_event_created_by_id == $user->getId() ? true : false;

				  $starts_on_time = array_var($subobject, 'starts_on_time', null);
				  if ($starts_on_time) {
				  ...
				  $result[] = array(
					  'id'              => $id,
					  'type'            => array_var($subobject, 'type'),
					  'parent_id'       => $parent_id,
					  'parent_type'     => array_var($subobject, 'parent_type'),
					  'name'            => array_var($subobject, 'name'),
					  'starts_on'       => array_var($subobject, 'starts_on'),
					  'ends_on'         => array_var($subobject, 'ends_on'),
					  'repeat'          => array_var($subobject, 'repeat_event', 'dont'),
					  'repeat_until'    => array_var($subobject, 'repeat_until'),
					  'starts_on_time'  => $starts_on_time,
					  'permissions'     => array(
						  'can_edit'        => $is_calendar_event_creator,
						  'can_trash'       => $is_calendar_event_creator,
						  'can_reschedule'  => $is_calendar_event_creator
					  ),
					  'urls'          => array(
