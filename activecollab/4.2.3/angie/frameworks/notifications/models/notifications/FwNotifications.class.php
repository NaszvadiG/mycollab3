  ...
  abstract class FwNotifications extends BaseNotifications {
    ...
    static function findRecentByUser(User $user, $since = null) {
      ...
      if($since instanceof DateTimeValue) {
        return Notifications::findBySQL("SELECT DISTINCT $notifications_table.* FROM $notifications_table, $recipients_table WHERE $notifications_table.id = $recipients_table.notification_id AND $notifications_table.created_on > ? AND $recipients_table.recipient_id = ? ORDER BY $notifications_table.created_on DESC LIMIT 0, 100", $since, $user->getId());
      } else {
        return Notifications::findBySQL("SELECT DISTINCT $notifications_table.* FROM $notifications_table, $recipients_table WHERE $notifications_table.id = $recipients_table.notification_id AND $recipients_table.recipient_id = ? ORDER BY $notifications_table.created_on DESC LIMIT 0, 100", $user->getId());
      } // if
    } // findRecentByUser
    ...
    static function findUnseenByUser(User $user, $limit = null) {
      ...
      return Notifications::findBySQL("SELECT DISTINCT $notifications_table.* FROM $notifications_table, $recipients_table WHERE $notifications_table.id = $recipients_table.notification_id AND $recipients_table.recipient_id = ? AND seen_on IS NULL ORDER BY $notifications_table.created_on DESC LIMIT 0, " . $limit, $user->getId());
    } // findUnseenByUser
