<?php
  ...
  abstract class FwNotification extends BaseNotification implements IRoutingContext, IReadOnly {
    ...
    function addRecipient($r) {
      ...
      if(is_foreachable($r)) {
        $notification_id = $this->getId();
        $batch = new DBBatchInsert(TABLE_PREFIX . 'notification_recipients', array('notification_id', 'recipient_id', 'recipient_name', 'recipient_email'));

        foreach($r as $recipient) {
          $batch->insert($notification_id, $recipient->getId(), $recipient->getName(), $recipient->getEmail());
        } // foreach
    ...
    function isThisNotificationVisibleInChannel(NotificationChannel $channel, IUser $recipient) {
      if($recipient instanceof User && !$channel->isEnabledFor($recipient)) {
        return false;
      } // if
