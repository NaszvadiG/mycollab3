  ...
  class FwAssignments {
    ...
    static function isAssignee(IUser $user, IAssignees $parent) {
      if($user instanceof User && $user->getState() >= STATE_ARCHIVED) {
        return Assignments::isResponsible($user, $parent) || ($parent->assignees()->getSupportsMultipleAssignees() && DB::executeFirstCell('SELECT COUNT(*) FROM ' . TABLE_PREFIX . 'assignments WHERE parent_type = ? AND parent_id = ? AND user_id = ?', get_class($parent), $parent->getId(), $user->getId()));
      } else {
        return false;
      } //if
    } // isAssignee
