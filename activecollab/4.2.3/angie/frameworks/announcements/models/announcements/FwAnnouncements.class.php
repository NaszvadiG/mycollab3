<?php
  ...
  class FwAnnouncements extends BaseAnnouncements {
    ...
    static function findActiveByUser(IUser $user) {
      ...
      $dismissed_announcement_ids = DB::executeFirstColumn("SELECT DISTINCT $announcement_dismissals_table.announcement_id FROM $announcements_table, $announcement_dismissals_table WHERE $announcements_table.id = $announcement_dismissals_table.announcement_id AND $announcements_table.expiration_type = ? AND $announcement_dismissals_table.user_id = ?", FwAnnouncement::ANNOUNCE_EXPIRATION_TYPE_UNTIL_DISMISSED, $user->getId());
